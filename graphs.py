...
def plot_error(err_cov_x, err_cov_y, err_cov_yaw=None,type='kf'):
    """
    ...
    if err_cov_yaw:
        ...
        temp = np.sign(err_yaw)
        err_yaw = np.sqrt(abs(err_yaw))*temp
        ...
        ax3.legend(["Error (yaw_gt - yaw_predicted) [rad]", "Predicted variance"], prop={"size": 20}, loc="best")
    show_graphs(base_dir,f"sigma error {type}")
...
def plot_trajectory_comparison_with_and_without_noise(enu, enu_noise, enu_predicted=None):
    ...
    ax.scatter(enu_noise[:, 0], enu_noise[:, 1] ,c='r',marker='.')
...
def plot_trajectory_comparison(enu, enu_predicted,type='KF'):
    """
    ...
    ax.plot(enu_predicted[:, 0], enu_predicted[:, 1], 'g')
    ax.set_aspect('equal', adjustable='box')
    title =f"Comparison of the ground truth trajectory and the predicted trajectory of the {type} " 
    ax.set_title(title, fontsize=20)
    ax.set_xlabel("East [meters]", fontsize=20)
    ax.set_ylabel("North [meters]", fontsize=20)
    ax.legend(["Ground truth trajectory", "Predicted trajectory"], prop={"size": 20}, loc="best")
    show_graphs(base_dir,title)
...
def plot_trajectory_comparison_dead_reckoning(enu, enu_predicted, enu_dead_reckoning):
    ...
    ax.set_aspect('equal', adjustable='box')
    title = "Comparison of the ground truth trajectory, the predicted trajectory and the dead reckoning trajectory"
...
def plot_single_graph(X_Y, title, xlabel, ylabel, label, is_scatter=False, sigma=None):
    ...
    plt.legend()
    show_graphs(folder= base_dir,file_name=title)
    #plt.show()
    #plt.imsavefig(base_dir + "/" + title+".png")
    #plt.savefig(base_dir + "/" + title+".png")
    #plt.close()
...
def plot_two_graphs_and_scatter(X_Y0, X_Y1, X_Y2, title, xlabel, ylabel, label0, label1, label2):
    plt.figure()
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.plot(X_Y0[:, 0], X_Y0[:, 1], 'b-', label=label0)
    plt.plot(X_Y1[:, 0], X_Y1[:, 1], 'g-', label=label1)
    plt.scatter(X_Y2[:, 0], X_Y2[:, 1], label=label2, s=1, c='r')
    plt.legend()
    show_graphs(base_dir,title)
...
def save_animation(ani, basedir, file_name):
    ...
    import matplotlib
    #writer = matplotlib.animation.PillowWriter(fps=50)
